...
app.delete(
  ...
  async (request, response) => {
    ...
    const { payload } = request;
    const { user_id, name, username, gender } = payload;
    const deleteUserQuery = `SELECT * FROM tweet WHERE tweet.user_id = ${user_id} AND tweet.tweet_id = ${tweetId};`;
    const tweetUser = await db.all(deleteUserQuery);
